    ...
    <div class="leftside-content-header">
        <ul class="breadcrumbs">
            <li><i class="fa fa-table" aria-hidden="true"></i><a
                    href="{{ Request::url() }}">Free Trial Subscribers</a></li>
...
<div class="row animated fadeInRight">
    <div class="col-sm-12">
        <h4 class="section-subtitle"><b>All Free Trial Subscribers</b></h4>
        <div class="panel">
            <div class="panel-content">
                <div class="table-responsive">
                    <table id="users_table"
                        ...
                        <tbody>
                            @foreach ($trialSubscribers as $trialSubscriber)
                            <tr>
                                <td class="text-center">{{ $trialSubscriber->customer_id }}</td>
                                <td class="text-center">
                                    {{ $trialSubscriber->first_name }} {{ $trialSubscriber->last_name }} <br>
                                    {{ $trialSubscriber->phone }} <br>
                                    {{ $trialSubscriber->email }}
                                </td>
                                <td class="text-center">{{ $trialSubscriber->facebook_link }}</td>
                                <td class="text-center">{{ $trialSubscriber->package_name }}</td>
                                <td class="text-center">{{ $trialSubscriber->product_category }}</td>
                                <td class="text-center">
                                    @php
                                    // create a $dt object with the UTC timezone
                                    $dt = new DateTime($trialSubscriber->updated_at,
