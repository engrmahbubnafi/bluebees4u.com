    ...
    <div class="leftside-content-header">
        <ul class="breadcrumbs">
            <li><i class="fa fa-table" aria-hidden="true"></i><a
                    href="{{ Request::url() }}">New Paid Subscribers</a>
            </li>
...
<div class="row animated fadeInRight">
    <div class="col-sm-12">
        <h4 class="section-subtitle"><b>All New Paid Subscribers</b></h4>
        <div class="panel">
            <div class="panel-content">
                <div class="table-responsive">
                    <table id="users_table"
                        ...
                        <tbody>
                            @foreach ($newPaidSubscribers as $newPaidSubscriber)
                            <tr>
                                <td class="text-center">{{ $newPaidSubscriber->customer_id }}</td>
                                <td class="text-center">
                                    {{ $newPaidSubscriber->first_name }}
                                    {{ $newPaidSubscriber->last_name }} <br>
                                    {{ $newPaidSubscriber->phone }} <br>
                                    {{ $newPaidSubscriber->email }}
                                </td>
                                <td class="text-center">{{ $newPaidSubscriber->facebook_link }}
                                </td>
                                <td class="text-center">{{ $newPaidSubscriber->package_name }}</td>
                                <td class="text-center">{{ $newPaidSubscriber->product_category }}
                                </td>
                                <td class="text-center">
                                    @php
                                    // create a $dt object with the UTC timezone
                                    $dt = new
                                    DateTime($newPaidSubscriber->updated_at,
