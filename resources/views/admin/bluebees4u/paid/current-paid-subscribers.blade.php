    ...
    <div class="leftside-content-header">
        <ul class="breadcrumbs">
            <li><i class="fa fa-table" aria-hidden="true"></i><a
                    href="{{ Request::url() }}">Current Paid Subscribers</a></li>
...
<div class="row animated fadeInRight">
    <div class="col-sm-12">
        <h4 class="section-subtitle"><b>All Current Paid Subscribers</b></h4>
        <div class="panel">
            <div class="panel-content">
                <div class="table-responsive">
                    <table id="users_table"
                        ...
                        <tbody>
                            @foreach ($currentPaidSubscribers as $currentPaidSubscriber)
                            <tr>
                                <td class="text-center">{{ $currentPaidSubscriber->customer_id }}</td>
                                <td class="text-center">
                                    {{ $currentPaidSubscriber->first_name }} {{ $currentPaidSubscriber->last_name }} <br>
                                    {{ $currentPaidSubscriber->phone }} <br>
                                    {{ $currentPaidSubscriber->email }} <br>
                                    {{ $currentPaidSubscriber->facebook_link }}
                                </td>
                                <td class="text-center">{{ $currentPaidSubscriber->package_name }}</td>
                                <td class="text-center">
                                    @php
                                    $dt = new DateTime($currentPaidSubscriber->updated_at,
                                ...
                                <td class="text-center">{{ $currentPaidSubscriber->product_category }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
