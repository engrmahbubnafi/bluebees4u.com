...
<div class="row animated fadeInRight">
    <div class="col-sm-12">
        <h4 class="section-subtitle"><b>Paid Honey Bee Subscribers</b></h4>
        <div class="panel">
            <div class="panel-content">
                <div class="table-responsive">
                    <table id="users_table"
                        ...
                        <tbody>
                            @foreach ($paidHoneyBeeSubscribers as $paidHoneyBeeSubscriber)
                            <tr>
                                <td class="text-center">{{ $paidHoneyBeeSubscriber->customer_id }}
                                </td>
                                <td class="text-center">{{ $paidHoneyBeeSubscriber->first_name }}
                                    {{ $paidHoneyBeeSubscriber->last_name }}
                                    <br>
                                    {{ $paidHoneyBeeSubscriber->phone }} <br>
                                    {{ $paidHoneyBeeSubscriber->email }}
                                </td>
                                <td class="text-center">{{ $paidHoneyBeeSubscriber->facebook_link }}
                                </td>
                                <td class="text-center">{{ $paidHoneyBeeSubscriber->product_category }}
                                </td>
                                <td class="text-center">
                                    @php
                                    // create a $dt object with the UTC timezone
                                    $dt = new
                                    DateTime($paidHoneyBeeSubscriber->updated_at,
